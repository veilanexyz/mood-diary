let date = new Date(); // creates a new date object with the current date and time
let year = date.getFullYear(); // gets the current year
let month = date.getMonth(); // gets the current month (index based, 0-11)

const day = document.querySelector(".calendar-dates"); // selects the element with class "calendar-dates"
const currdate = document.querySelector(".calendar-current-date"); // selects the element with class "calendar-current-date"
const prenexIcons = document.querySelectorAll(".calendar-navigation span"); // selects all elements with class "calendar-navigation span"

const months = [
  "Январь",
  "Февраль",
  "Март",
  "Апрель",
  "Май",
  "Июнь",
  "Июль",
  "Август",
  "Сентябрь",
  "Октябрь",
  "Ноябрь",
  "Декабрь"
]; // array of month names

// function to generate the calendar
const manipulate = () => {
  let dayone = (new Date(year, month, 1).getDay() + 6) % 7; // Adjusted to start on Monday
  let lastdate = new Date(year, month + 1, 0).getDate();
  let dayend = (new Date(year, month, lastdate).getDay() + 6) % 7; // Adjusted to start on Monday
  let monthlastdate = new Date(year, month, 0).getDate();
  let calendarDatesContainer = document.querySelector('.calendar-dates-container');
  calendarDatesContainer.innerHTML = ''; // Clear previous content

  let lit = [];
  for (let i = dayone; i > 0; i--) {
    lit.push(`<li class="inactive">${monthlastdate - i + 1}</li>`);
  }

  for (let i = 1; i <= lastdate; i++) {
    let isToday = i === date.getDate() && month === new Date().getMonth() && year === new Date().getFullYear() ? "active" : "";
    lit.push(`<li class="${isToday}">${i}</li>`);
  }

  for (let i = dayend; i < 6; i++) {
    lit.push(`<li class="inactive">${i - dayend + 1}</li>`);
  }

  // Generate rows with lines and circles
  while (lit.length) {
    let row = lit.splice(0, 7);
    let rowHtml = `<div class="line"></div><ul class="calendar-dates-row">${row.join('')}</ul><div class="circles-row">${row.map((item, index) => {
      if (item.includes('inactive')) {
        return '<div class="circle inactive"></div>';
      } else {
        return '<div class="circle"></div>';
      }
    }).join('')}</div>`;
    calendarDatesContainer.innerHTML += rowHtml;
  }

  currdate.innerText = `${months[month]} ${year}`;
};

manipulate();

// Attach a click event listener to each icon
prenexIcons.forEach(icon => {
  // When an icon is clicked
  icon.addEventListener("click", () => {
    // Check if the icon is "calendar-prev" or "calendar-next"
    month = icon.id === "calendar-prev" ? month - 1 : month + 1;

    // Check if the month is out of range
    if (month < 0 || month > 11) {
      // Set the date to the first day of the month with the new year
      date = new Date(year, month, new Date().getDate());
      // Set the year to the new year
      year = date.getFullYear();
      // Set the month to the new month
      month = date.getMonth();
    } else {
      // Set the date to the current date
      date = new Date();
    }

    // Call the manipulate function to update the calendar display
    manipulate();
  });
});
